...
public record IconSpec(float red, float green, float blue, Identifier background, Identifier logo) {
    ...
    private static final Identifier BACKGROUND = texture("back");
    private static final Identifier QUESTION = texture("question");
    ...
    public static IconSpec create(EnchantmentInfo info) {
        ...
        Identifier logo = SUPPORTED.contains(path) ? texture(path) : QUESTION;
        return new IconSpec(color[0], color[1], color[2], BACKGROUND, logo);
    }

    private static Identifier texture(String name) {
        return Identifier.fromNamespaceAndPath("visible-librarian-trades", "textures/entity/villager/enchant_icons/" + name + ".png");
    }
}
